...
$remoteCmd = 'source /home/ngankmnx/nodevenv/website/20/bin/activate && cd /home/ngankmnx/website && PREV_REV=$(git rev-parse HEAD) && git pull origin main && NEW_REV=$(git rev-parse HEAD) && CHANGED=$(git diff --name-only $PREV_REV $NEW_REV | grep -E "package(-lock)?\.json") && if [ -n "$CHANGED" ]; then echo "Dependencies updated, running npm ci..."; npm ci --omit=dev; fi && touch tmp/restart.txt && echo "Passenger restart triggered."'
